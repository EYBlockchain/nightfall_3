...
const computePrivateInputsNullifiers = (
  ...
  const paddedPaths: GeneralNumber[][] = padArray(paths, new Array(32).fill(0), numberNullifiers);
  const paddedOrders: GeneralNumber[] = padArray(orders, 0, numberNullifiers);
  const paddedRootKeys: GeneralNumber[] = padArray(rootKey, 0, numberNullifiers);
