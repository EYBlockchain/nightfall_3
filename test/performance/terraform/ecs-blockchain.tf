resource "aws_lb" "performance_test_blockchain" {
  count              = var.DEPLOY_BLOCKCHAIN == "true" ? 1 : 0
  name               = "performance-test-blockchain-lb"
...
resource "aws_lb_listener" "performance_test_blockchain" {
  count             = var.DEPLOY_BLOCKCHAIN == "true" ? 1 : 0
  load_balancer_arn = aws_lb.performance_test_blockchain[count.index].id
  port              = 8546
  protocol          = "HTTP"

  default_action {
    target_group_arn = aws_lb_target_group.performance_test_blockchain[count.index].id
    type             = "forward"
  }
}
  ...
  container_definitions = jsonencode([
    {
      ...
      "logConfiguration" : { 
        "logDriver": "awslogs",
        "options": { 
            "awslogs-group" : "${aws_cloudwatch_log_group.performance_test_blockchain[count.index].name}",
...
resource "aws_ecs_service" "nightfall-blockchain" {
  ...
  task_definition        = aws_ecs_task_definition.nightfall-blockchain[count.index].id
  ...
  network_configuration {
    security_groups = [aws_security_group.performance_test_nightfall-blockchain-sg[count.index].id]
