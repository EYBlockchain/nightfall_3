...
const transpileClassesNf3Sdk = _pathToSrc => {
  ...
  const regexMakeBlock =
    /async makeBlockNow\(\) {\n(\s)*return axios.post\(`\${this.optimistBaseUrl}\/block\/make-now`\);\n(\s)*}/g;
...
copyDir('./common-files/', './test/adversary/adversary-common-files/').then(() => {
  console.log('done with common-files copy');

  transpileClassesNf3Sdk('./test/adversary/adversary-common-files/classes/nf3.mjs');

  console.log(`transpile adversary common-files done`);
});
