...
const transpileBlockAssembler = _pathToSrc => {
  ...
  const regexReplaceMempoolFunction =
    /const mempoolTransactions = await getSortedByFeeMempoolTransactions\(\);/g;
  const reMempoolFunction =
    'const mempoolTransactions = await getSortedByFeeMempoolTransactions(badTxType);';
...
const transpileLibNf3Sdk = (_pathToSrc, _pathToInject) => {
  ...
  const [postAmble] = injectFile.match(regexInjectFileNoPreamble);
  // const regexReplaceCall = /(if \(type === 'rollback'\) proposeEmitter.emit\('rollback', data\);)/g;
