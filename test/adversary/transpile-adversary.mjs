...
const transpileBlockProposed = _pathToSrc => {
  ...
  const reComponent = `logger.info(\`NONSTOP_QUEUE_AFTER_INVALID_BLOCK: \${nonStopQueueFlag}\`);
      if (!nonStopQueueFlag)
  ...
  const regexReplaceComponents2 = /(let ws;)/g;
  const reComponent2 = `let ws; 
let nonStopQueueFlag = process.env.NONSTOP_QUEUE_AFTER_INVALID_BLOCK;
export function setNonStopFlag(flag) {
  nonStopQueueFlag = flag;
}`;
  srcFile = `${srcFile.replace(regexReplaceComponents2, reComponent2)}`;
...
import { setNonStopFlag } from '../event-handlers/block-proposed.mjs';
${srcFile}`;
