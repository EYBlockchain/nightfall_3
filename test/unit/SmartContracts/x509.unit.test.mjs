...
const AUTHORITY_KEY_IDENTIFIER = process.env.AUTHORITY_KEY_IDENTIFIER || `0x${'ef355558d6fdee0d5d02a22d078e057b74644e5f'.padStart(
  64,
  '0',
)}`;
...
const END_USER_PRIV_KEY_PATH = process.env.END_USER_PRIV_KEY_PATH || 'test/unit/utils/Nightfall_end_user.der';
const END_USER_CERTIFICATE_PATH = process.env.END_USER_CERTIFICATE_PATH || 'test/unit/utils/Nightfall_end_user.cer';
const SKIP_INTERMEDIATE_CERTIFICATE_CHECK = process.env.SKIP_INTERMEDIATE_CERTIFICATE_CHECK;
...
describe('DerParser contract functions', function () {
  ...
  it('Should parse the intermediate CA cert DER encoding', async function () {
    if (SKIP_INTERMEDIATE_CERTIFICATE_CHECK === 'true') return;
  ...
  it('Should fail to validate the user certificate until it has validated the intermediate CA cert', async function () {    
    if (SKIP_INTERMEDIATE_CERTIFICATE_CHECK === 'true') {
