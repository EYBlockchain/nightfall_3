...
describe('Testing the challenge http API', () => {
  ...
  before(async () => {
    ...
    let res;
    let txToSign;
    let counter = 1; // to edit a block to a different bad block type each time a block proposed transaction is received
        ...
        try {
          if (type === 'block') {
            ...
            if (counter === 1) {
              ...
              topicsFaultyBlockZero = res.block.blockHash;
  ...
  describe('Create challenge block consisting of two deposit transactions ', () => {
    it('should create a block consisting of two deposits', async () => {
  ...
  describe('Rollback Challenge Test', () => {
    ...
    describe('Challenges: Incorrect root and Incorrect leaf challenges', () => {
      it('Should delete the flawed block and rollback the leaves', async () => {
        await testForEvents(stateAddress, [
          ...
          web3.eth.abi.encodeParameter('bytes32', topicsFaultyBlockZero),
        ]);
      });
    });
  });
