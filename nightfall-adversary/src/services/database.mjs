/**
Functions for storing blockchain data that the adversary application needs to
...
/**
Function to return 'number' transactions which are already on chain. If there
are fewer than 'number' transactions, all are returned.
*/
export async function getTransactionsOnChain(number) {
  const connection = await mongo.connection(MONGO_URL);
  const db = connection.db(ADVERSARY_DB);
  return db
    .collection(TRANSACTIONS_COLLECTION)
    .find({ mempool: false, blockNumberL2: { $ne: -1 } }, { limit: number, projection: { _id: 0 } })
    .toArray();
}

/**
Function to return 'number' transactions which are already on chain that are either transfers or withdraws.
If there are fewer than 'number' transactions, all are returned.
*/
export async function getSpentTransactionsOnChain(number) {
  const connection = await mongo.connection(MONGO_URL);
  const db = connection.db(ADVERSARY_DB);
  return db
    .collection(TRANSACTIONS_COLLECTION)
    .find(
      { mempool: false, transactionType: { $in: ['1', '2', '3'] }, blockNumberL2: { $ne: -1 } },
      { limit: number, projection: { _id: 0 } },
    )
    .toArray();
}
