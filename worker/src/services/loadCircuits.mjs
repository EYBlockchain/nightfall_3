...
export default async function loadCircuits(circuits) {
  ...
  if (!circuits.name.endsWith('.tar') && !circuits.name.endsWith('.circom')) {
  ...
  if (circuits.name.endsWith('.tar')) {
    ...
    // check the archive contained .circom files only
    files.forEach(file => {
      if (!file.endsWith('.circom')) {
        throw Error('For shame, this is not an .circom file');
      }
      return null;
    });
  }
