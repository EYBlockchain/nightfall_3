...
const StateV2 = artifacts.require('StateV2.sol');
...
module.exports = async function(deployer) {
  ...
  await upgradeProxy((await Structures.deployed()).address, Structures, {deployer, unsafeAllowLinkedLibraries: true });
  await upgradeProxy((await Config.deployed()).address, Config, {deployer, unsafeAllowLinkedLibraries: true });
  await upgradeProxy((await Proposers.deployed()).address, Proposers, {deployer, unsafeAllowLinkedLibraries: true });
  await upgradeProxy((await Challenges.deployed()).address, Challenges, {deployer, unsafeAllowLinkedLibraries: true });
  await upgradeProxy((await Shield.deployed()).address, Shield, {deployer, unsafeAllowLinkedLibraries: true });
  await upgradeProxy((await State.deployed()).address, StateV2, {deployer, unsafeAllowLinkedLibraries: true });
};
