...
library MerkleTree_Stateless {
    ...
    */
    ...
    uint256 constant TREE_HEIGHT = 32; //change back to 32 after testing
    uint256 constant TREE_WIDTH = 2**TREE_HEIGHT; // 2 ** TREE_HEIGHT
    /*
    ...
    */
        ...
        )
    {
        ...
        require(TREE_WIDTH > _leafCount, "There is no space left in the tree.");

        uint256 slot = getFrontierSlot(_leafCount);
        uint256 nodeIndex = _leafCount + TREE_WIDTH - 1;
    ...
    */
        ...
        )
    {
        ...
        // check that space exists in the tree:
        require(TREE_WIDTH > _leafCount, "There is no space left in the tree.");
        if (numberOfLeaves > TREE_WIDTH - _leafCount) {
            uint256 numberOfExcessLeaves = numberOfLeaves - (TREE_WIDTH - _leafCount);
            ...
            numberOfLeaves = TREE_WIDTH - _leafCount;
        }
        ...
        for (uint256 level = slot + 1; level <= TREE_HEIGHT; level++) {
            if (nodeIndex % 2 == 0) {
                // even nodeIndex
