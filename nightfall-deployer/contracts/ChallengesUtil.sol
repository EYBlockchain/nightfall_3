...
library ChallengesUtil {
  ...
  function libChallengeNewRootCorrect(
    ...
    uint256 l;
    for (uint256 i = 0; i < priorBlockTransactions.length; i++) {
      for (uint256 j = 0; j < priorBlockTransactions[i].commitments.length; j++)
    ...
    for (uint256 i = 0; i < transactions.length; i++) {
      for (uint256 j = 0; j < transactions[i].commitments.length; j++)
      if (transactions[i].commitments[j] != ZERO)
      commitments[k++] = transactions[i].commitments[j];
    }
    // At last, we can check if the root itself is correct!
    (bytes32 root, , ) =
    MerkleTree_Stateless.insertLeaves(commitments, _frontier, commitmentIndex);
  ...
  pure
  {
    ...
    uint256 nZeroCommitments;
    for (uint256 i = 0; i < transaction.commitments.length; i++) {
      if (transaction.commitments[i] == ZERO) nZeroCommitments++;
    }
    uint256 nZeroNullifiers;
    for (uint256 i = 0; i < transaction.nullifiers.length; i++) {
      if (transaction.nullifiers[i] == ZERO) nZeroNullifiers++;
    }
    uint256 nZeroProof;
