...
contract State is Structures, Config, Initializable, ReentrancyGuardUpgradeable {
    ...
    function initialize(
        ...
        if (_proposersAddress != address(0)) proposersAddress = _proposersAddress;
        if (_challengesAddress != address(0)) challengesAddress = _challengesAddress;
        if (_shieldAddress != address(0)) shieldAddress = _shieldAddress;
    }
    ...
    function setProposer(address addr, LinkedAddress memory proposer) external onlyRegistered {
        proposers[addr] = proposer;
    }

    function getProposer(address addr) external view returns (LinkedAddress memory) {
        return proposers[addr];
    }

    function deleteProposer(address addr) external onlyRegistered {
        delete proposers[addr];
    }

    function setCurrentProposer(address proposer) external onlyRegistered {
        currentProposer = proposers[proposer];
    }

    function getCurrentProposer() external view returns (LinkedAddress memory) {
        return currentProposer;
    }

    function pushBlockData(BlockData memory bd) external onlyRegistered {
        blockHashes.push(bd);
    }

    function popBlockData() external onlyRegistered returns (BlockData memory) {
    ...
    function getBlockData(uint256 blockNumberL2) external view returns (BlockData memory) {
  ...
  reverse lookups
  */
    function getAllBlockData() external view returns (BlockData[] memory) {
        return blockHashes;
    }

    function getNumberOfL2Blocks() external view returns (uint256) {
    ...
    function addPendingWithdrawal(address addr, uint256 amount) external onlyRegistered {
    ...
    function getProposerStartBlock() external view returns (uint256) {
    ...
    function getBondAccount(address addr) external view returns (TimeLockedBond memory) {
    ...
    function updateBondAccountTime(address addr, uint256 time) external onlyRegistered {
