...
export function checkDuplicateCommitmentsWithinBlock(block, transactions) {
  ...
  blockCommitments.forEach((blockCommitment, index) => {
    const lastIndex = blockCommitment.lastIndexOf(blockCommitment);
    if (
      blockCommitment !== ZERO &&
      blockCommitment.indexOf(blockCommitment) !== blockCommitment.lastIndexOf(blockCommitment)
      // index !== blockCommitment.lastIndexOf(blockCommitment)
    ) {
      throw new BlockError(
...
export function checkDuplicateNullifiersWithinBlock(block, transactions) {
  ...
  blockNullifiers.forEach((blockNullifier, index) => {
    const lastIndex = blockNullifier.lastIndexOf(blockNullifier);
    if (
      blockNullifier !== ZERO &&
      blockNullifier.indexOf(blockNullifier) !== blockNullifier.lastIndexOf(blockNullifier)
      // index !== blockNullifier.lastIndexOf(blockNullifier)
    ) {
      throw new BlockError(
