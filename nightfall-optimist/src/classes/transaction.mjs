...
class Transaction {
  ...
  constructor({
    ...
    secrets: _secrets, // this must be array of objects that are compressed from Secrets class
    ...
    if (_secrets === undefined) secrets = [{ compressedSecrets: 0 }];
    // else if (_secrets.length === 1) secrets = [..._secrets, { hash: 0 }];
    else secrets = _secrets;
    ...
    const preimage = generalise({
      ...
      compressedSecrets: secrets.map(s => s.compressedSecrets),
      proof: flatProof,
    }).all.hex(32);
    // compute the solidity hash, using suitable type conversions
