...
import { LeafService, NodeService, MetadataService, HistoryService } from './db/service/index.mjs';
import { responseFunctions } from './filter-controller.mjs'; // eslint-disable-line import/no-cycle
...
async function rollback(db, treeHeight, leafCount) {
  ...
  if (history === null) {
    logger.debug(`There is no history for a leafcount of ${leafCount} so nothing to roll back to`);
    return;
  }
