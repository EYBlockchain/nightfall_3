...
function mulMod(timesMe, m = BN128_GROUP_ORDER) {
  return timesMe.reduce((e, acc) => (((e + m) % m) * acc) % m, BigInt(1));
}

function powerMod(base, exponent, m = BN128_GROUP_ORDER) {
...
function squareRootModPrime(n, p = BN128_GROUP_ORDER) {
  ...
  // eslint-disable-next-line no-constant-condition
  while (true) {
    if (t % p === BigInt(1) && R % p <= (p - BigInt(1)) / BigInt(2)) return R;
    if (t % p === BigInt(1) && R % p > (p - BigInt(1)) / BigInt(2)) return p - R;
